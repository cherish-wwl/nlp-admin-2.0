<template>
  <div class="grid-content">
    <span>图片类型：</span>
    <el-select v-model="searchType" placeholder="请选择图片类型" @:change="querySearch">
    ...
    <el-button size="small" type="primary" @click="querySearch"> 查询 </el-button>
    &nbsp;
    <el-button size="small" type="primary" @click="addRowData('add')">添加</el-button>
    <el-table v-loading="loading" :data="tableData" border style="width: 100%">
      <el-table-column type="index" width="50" />
      <el-table-column prop="fileDesc" label="图片名称" min-width="180" />
      <el-table-column prop="fileType" label="图片类别" min-width="180">
        <template slot-scope="scope">
          <span
            v-for="item in dictList"
            v-if="item.parentCode =='013'&&item.dictCode ==scope.row.fileType"
            :key="item.dictCode">{{ item.dictName }}</span>
        </template>
      </el-table-column>
      ...
      <el-form ref="ruleForm" :model="form" label-position="right" label-width="80px">
        ...
        <el-form-item label="图片">
          <el-upload
            ...
            <el-input v-model="form.fileUrl" prop="fileUrl" style="display:none;" />
          </el-upload>
        </el-form-item>
        <el-form-item label="图片名称">
          <el-input v-model="form.fileDesc" prop="fileDesc" />
        </el-form-item>
      </el-form>
